...
if __name__ == "__main__":
    # --- Dummy Data ---
    current_step_example = 3
    conversation_history_example = [
        HumanMessage(content="Hi VINO, let's start defining my project."),
        AIMessage(content="Great! Let's begin with Step 1: Initial State. What is the starting point or initial state of your project?"),
        HumanMessage(content="It's a project to build a new AI assistant for students called VINO."),
        AIMessage(content="Okay, VINO AI assistant for students is the initial state. Now for Step 2: Connection / Awareness. What key elements need to be connected for VINO to function?"),
        HumanMessage(content="We need a frontend GUI, a backend API, a vector database, and an LLM."),
        AIMessage(content="Excellent. Frontend, Backend, Vector DB, and LLM are the connected elements. Now we're at Step 3: Emergence / Initial Plan. Based on our discussion, let's draft the initial 6-step plan using the Universal Matrix framework. What should be the main goal for each step?")
    ]
    user_question_example = "Okay, for Step 1, the goal is 'Define Project Scope'. For Step 2, 'Identify Core Components'. For Step 3, 'Draft Initial Plan'. What about the others?"
    step_context_example = "Retrieved context mentioning project planning frameworks..." # From DB query based on "Step 3" + user query
    general_context_example = "User uploaded VINO_FRD_Students.pdf, VINO_ProjectPlan_Students.pdf" # From DB query based on user query
    planner_state_example = None # Planner not defined yet

    # --- Get the Prompt ---
    prompt = get_universal_matrix_prompt(
        current_step=current_step_example,
        history=conversation_history_example, # History obj passed separately
        question=user_question_example,
        step_context=step_context_example,
        general_context=general_context_example,
        planner_state=planner_state_example
    )
    ...
    if prompt:
        ...
        # --- Simulating Langchain Invocation (How you'd use it) ---
        # from langchain_google_genai import ChatGoogleGenerativeAI
        # model = ChatGoogleGenerativeAI(model="gemini-pro") # Or your model
        # chain = prompt | model
        # response = chain.invoke({
        #     "history": conversation_history_example, # Pass history here
        #     # Other variables are already baked into the prompt content by get_universal_matrix_prompt
        # })
        # print("\n--- Simulated LLM Response (Structure) ---")
        # print(response.content) # Fictional response
        print("\n--- Example Input Variables for .invoke ---")
        # Note: The variables like question, context etc. are now part of the
        # formatted HumanMessage content within the template.
        # You ONLY need to pass the 'history' variable when invoking.
        print(f"history: {conversation_history_example}")


    # --- Example for Step 4 (after planner exists) ---
    current_step_example = 4
    planner_state_example = """
    Planner Draft:
    1. Define Project Scope (Initial State) - DONE
    2. Identify Core Components (Connection) - DONE
    3. Draft Initial Plan (Emergence) - DONE (Draft below)
    4. Analyze Possibilities (Manifestation) - CURRENT
    5. Choose Direction (Vector) - TODO
    6. Define Boundaries (Completion) - TODO
    """
    user_question_example = "Looking at the plan, what possibilities does the current architecture allow?"
    step_context_example = "Retrieved context about system architecture options..."
    general_context_example = "User uploaded VINO_FRD_Students.pdf, VINO_ProjectPlan_Students.pdf, system-design.md"

    prompt_step4 = get_universal_matrix_prompt(
        current_step=current_step_example,
        history=conversation_history_example, # Assume history continues
        question=user_question_example,
        step_context=step_context_example,
        general_context=general_context_example,
        planner_state=planner_state_example
    ...
    if prompt_step4:
         print("\n\n--- Generated Prompt Template for Step 4 ---")
         print(prompt_step4)
         print("\n--- Example Input Variables for .invoke (Step 4) ---")
         print(f"history: {conversation_history_example}") # Still only need history